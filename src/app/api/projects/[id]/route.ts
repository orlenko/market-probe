...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Get project with page configs and analytics
    const project = await prisma.project.findUnique({
      where: { id: projectId },
      include: {
        pageConfigs: { where: { isActive: true } },
      },
    });
...
export async function DELETE(
  ...
  try {
    ...
    const hasData =
      existingProject._count.formSubmissions > 0 || existingProject._count.analyticsEvents > 0;
